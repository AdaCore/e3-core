...
class Walk(object):
    ...
    :vartype actions: DAG
    :ivar prev_fingerprint_cache: A dict of e3.anod.Fingerprint objects,
        indexed by the corresponding job ID. This dictionary contains
        the fingerprints of previous executions of actions from our DAG
        (self.actions).
    :vartype prev_fingerprint_cache: dict
    :ivar pending_fingerprints: A dict of e3.anod.Fingerprint objects,
        indexed by the corresponding job ID. This dictionary is
        a temporary store for computed fingerprints, between the moment
        we start a job, and the moment we collect its results. Once
        the results are collected, the entry should be removed.
    :vartype pending_fingerprints: dict
    ...
    def get_job(self, uid, data, predecessors, notify_end):
        ...
        :rtype: Job
        """
        # Get the previous fingerprint, if any, and then invalidate it.
        if uid in self.prev_fingerprint_cache:
            prev_fingerprint = self.prev_fingerprint_cache[uid]
        else:
            prev_fingerprint = self.load_previous_fingerprint(uid)
        self.__update_fingerprint(uid, None)
        self.pending_fingerprints[uid] = self.compute_new_fingerprint(uid)
    ...
    def collect(self, job):
        ...
        :type job: ProcessJob
        """
        # Update the action's fingerprint depending on how the job went:
        # If the job went as expected (success or skipped), save
        # the fingerprint we computed for it. If not, invalidate
        # the action's fingerprint to make sure we try that action
        # again next time.
        new_fingerprint = self.pending_fingerprints[job.uid]
        if job.status in (ReturnValue.success,
                          ReturnValue.force_skip,
                          ReturnValue.skip):
            self.__update_fingerprint(job.uid, new_fingerprint)
        else:
            self.__update_fingerprint(job.uid, None)
        ...
        if not requeued:
            del self.pending_fingerprints[job.uid]

        return requeued

    def __update_fingerprint(self, uid, fingerprint):
        """Update the fingerprint in our cache and call self.save_fingerprint.

        This is a convenience method we should call every time a job's
        fingerprint should be updated. This helps us make sure that
        our fingerprint cache and any fingerprint storage implemented
        by child classes remain in sync.
        ...
        :rtype: None
        """
        self.prev_fingerprint_cache[uid] = fingerprint
        self.save_fingerprint(uid, fingerprint)
