...
def test_svn_repo(svn) -> None:
    ...
    project_path = cwd / "test_project"
    mkdir(project_path)
    mkdir(project_path / "trunk")
    hello_relative_path = Path("trunk", "hello.txt")
    hello_path = str(project_path / hello_relative_path)
    echo_to_file(hello_path, "hello")

    # --- create a SVN repository from that project
    repos_path = str(cwd / "repos")
    ...
    working_copy_a_path = cwd / "working_copy_a"
    svn_a = SVNRepository(str(working_copy_a_path))
    ...
    echo_to_file(working_copy_a_path / hello_relative_path, "bye")
    ...
    working_copy_b_path = cwd / "working_copy_b"
    svn_b = SVNRepository(str(working_copy_b_path))
    svn_b.update(project_url)
    foo_path = working_copy_b_path / "trunk" / "foo"
    touch(foo_path)
    hello_b_path = working_copy_b_path / hello_relative_path
    ...
    working_copy_c_path = cwd / "working_copy_c"
    svn_c = SVNRepository(str(working_copy_c_path))
    ...
    bar_path = working_copy_c_path / "bar"
    touch(bar_path)
    # verify failures without the force option
    with pytest.raises(SVNError) as err:
        svn_c.update(url=project_url)
    assert "not empty" in str(err)
    touch(working_copy_c_path / ".svn")
    with pytest.raises(SVNError):
        svn_c.update(url=project_url)
