...
class TestSourceClosure(object):
    ...
    def get_source_closure(self, name, expand_packages=True,
                           other_builds=None):
    ...
    def test_simple_source_closure(self):
        ...
        assert len(unpublished_sources) == 1 \
    ...
    def test_recursive_source_closure_with_package(self):
        ...
        for key in sc.package_list:
            ...
            sc.resolve_package(key.anod_uid,
                               [('%s-package-src' % name, True),
                                ('%s-package-int-src' % name, False)])
